...
namespace Onyx
{
        ...
        : m_InstanceSize{ instanceSize }, m_InstanceCount{ instanceCount }, m_UsageFlags{ usageFlags }, m_MemoryPropertyFlags{ memoryPropertyFlags }
    {
        ...
        Application::Get().GetDevice().CreateBuffer(m_BufferSize, m_UsageFlags, m_MemoryPropertyFlags, m_Buffer, m_Memory);
    }

    Buffer::~Buffer() 
    {
        Unmap();
        vkDestroyBuffer(Application::Get().GetDevice().GetHandle(), m_Buffer, nullptr);
        vkFreeMemory(Application::Get().GetDevice().GetHandle(), m_Memory, nullptr);
    }
    ...
    {
        ONYX_CORE_ASSERT(m_Buffer && m_Memory, "Called map on buffer before create");
        return vkMapMemory(Application::Get().GetDevice().GetHandle(), m_Memory, offset, size, 0, &m_Mapped);
    }
    ...
    void Buffer::Unmap() 
    {
        if (m_Mapped) 
        {
            vkUnmapMemory(Application::Get().GetDevice().GetHandle(), m_Memory);
            m_Mapped = nullptr;
        }
    }
    ...
    {
        ONYX_CORE_ASSERT(m_Mapped, "Cannot copy to unmapped buffer");
    ...
    VkResult Buffer::Flush(VkDeviceSize size, VkDeviceSize offset) 
    {
        ...
        return vkFlushMappedMemoryRanges(Application::Get().GetDevice().GetHandle(), 1, &m_MappedRange);
    }
    ...
    VkResult Buffer::Invalidate(VkDeviceSize size, VkDeviceSize offset) 
    {
        ...
        m_MappedRange.offset = offset;
        m_MappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(Application::Get().GetDevice().GetHandle(), 1, &m_MappedRange);
    }
